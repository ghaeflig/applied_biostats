...
Two variables were determined by other variables: the available capacity corresponds to the revenue tons per aircraft mile divided by the ton-mile load factor, and the adjusted assets are equal to the difference between the total assets and the investments and special funds. Therefore, due these dependencies, RTM, LF, TA and I are removed for the following analysis. In this manner, six independent variables are used to explained the total operating cost. 
...
```{r, fig.width=6, fig.height =6, echo=FALSE, fig.cap="Bivariate scatter plots for every relations."}
pairs(data_log)
...
Finally, residuals vs. leverage and cook's distance plots allow to identify potential influencor outliers. No airline errors are higher than 1. Central data seems to have a small deviation taht can influence the regression, but it stays reasonable. 
