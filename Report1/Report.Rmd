...
The analysis is interested in airline costs depending on multiple factors. These latter are evaluated in order to evaluate their influences on the cost. 
<!-- (all variables defined:) -->
The data set consists of 11 variables for 31 airlines. The dependent variable y is the total operating costs (TOC). Its unit of measurement is cents per revenue ton-mile. To define it, 10 other variables are available: the length of the flight, the speed of the plane, the daily flight time, the metropolitan population served, the revenue tons per aircraft mile, the ton-mile load factor, the available capacity, the total assets, the investments and special funds, and the adjusted assets. 
Tow variables were determined by other variables: The capacity corresponds to the division of the revenue tons per aircraft mile with the ton-mile load factor, and the adjusted assets are equal to the difference between the total assets and the investments and special funds. 
...
We can see in the left the boxplot that the factors of the raw data are unequally distributed ; they don't have the same magnitude of values. This is a problem to predict a model because some factors due to the fact that they have bigger values can have a non-realistic weight and therefore, it can lead to a meaningless regression. In order to get the data in the same order of magnitude, we decided to take the log of it except for the factor LF. Indeed, this factor has already a low magnitude (mean=0.48) without taking the log of it.
...
```{r, fig.width=3.5, fig.height =3.5, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Correlation values between each features."}
#Numerical part
...
```{r, fig.width=6, fig.height =6, echo=FALSE, fig.cap="Scatter plots for every relations."}
pairs(data_log)
